...
export const ACTION_TYPES = {
  SEARCH_ASSIGNMENTS: 'assignment/SEARCH_ASSIGNMENTS',
...
export default (state: AssignmentState = initialState, action): AssignmentState => {
  switch (action.type) {
    case REQUEST(ACTION_TYPES.SEARCH_ASSIGNMENTS):
    ...
    case FAILURE(ACTION_TYPES.SEARCH_ASSIGNMENTS):
    ...
    case SUCCESS(ACTION_TYPES.SEARCH_ASSIGNMENTS):
      return {
        ...state,
        loading: false,
...
const apiSearchUrl = 'api/_search/assignments';

// Actions

export const getSearchEntities: ICrudSearchAction<IAssignment> = query => ({
  type: ACTION_TYPES.SEARCH_ASSIGNMENTS,
  payload: axios.get<IAssignment>(`${apiSearchUrl}?query=` + query)
});

export const getEntities: ICrudGetAllAction<IAssignment> = (page, size, sort) => ({
  type: ACTION_TYPES.FETCH_ASSIGNMENT_LIST,
  payload: axios.get<IAssignment>(`${apiUrl}?cacheBuster=${new Date().getTime()}`)
});
